...
QEMU_BIN="/run/media/stray/myfuckinghd/apolloOS/vgpu/qemu/build/qemu-system-x86_64"
ISO="$1"
RAM="${2:-2G}"
...
if [ -z "$ISO" ]; then
    ...
    echo "                   amd-rx480, amd-rx6700xt, intel-arc-a770"
    exit 1
fi
...
exec "$QEMU_BIN" \
    ...
    -kernel "$ISO" \
    -initrd "/run/media/stray/myfuckinghd/apolloOS/vgpu/initrd" \
